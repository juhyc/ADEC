...
writer = SummaryWriter('runs/train_disp_raft_gru')
...
def train(args):
    model = nn.DataParallel(RAFTStereo(args), device_ids=[0,1])
    ...
    while should_keep_training:
        ...
        for i_batch, (img_list, *data_blob) in enumerate(tqdm(train_loader)):
            optimizer.zero_grad()
            img1_left, img1_right, _, _, disp, valid = [x.cuda() for x in data_blob]
            ...
            disp_predictions = model(img1_left, img1_right)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='finetuned_raftstereo_gru_carla', help="name your experiment")

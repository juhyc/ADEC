...
writer = SummaryWriter('runs/test_sequence_calra_comparison')
...
def test_sequence(args):
    ...
    logging.info(f"Loading averageAE_raft checkpoint... {args_averageAE_raft.restore_ckpt}")
    averageAE_raft.load_state_dict(averageAE_raft_ckpt, strict=False)
    logging.info(f"Done averageAE_raft loading checkpoint...{args.restore_ckpt}")
    averageAE_raft.cuda()
    averageAE_raft.eval()
    ...
    logging.info(f"Loading gradientAE_raft checkpoint... {args_gradientAE_raft.restore_ckpt}")
    gradientAE_raft.load_state_dict(gradientAE_raft_ckpt, strict=False)
    logging.info(f"Done gradientAE_raft loading checkpoint...{args.restore_ckpt}")
    ...
    test_step = 0
    base_output_folder = 'review_comparison_finetuned_model'

...
def train(args):
    ...
    while should_keep_training:
        ...
        for i_batch, (img_list, *data_blob) in enumerate(tqdm(train_loader)):
            ...
            exp1 = generate_random_exposures(args.batch_size, value=2)
            exp2 = generate_random_exposures(args.batch_size, value=2)
            # exp1 = generate_random_exposures(args.batch_size, valid_mode=True, value=4.0)
            # exp2 = generate_random_exposures(args.batch_size, valid_mode=True, value=0.8)

            assert model.training
            disp_predictions, fmap1, fmap1_next, warped_fmap_left, flow_L, fused_fmap1, cap1, cap1_next = model(img1_left, img1_right, img2_left, img2_right, exp1, exp2)
            ...
            if total_steps % training_frequency == training_frequency - 1:
                ...
                writer.add_image('Train/Left Cap1', cap1[0]**(1/2.2), total_steps/training_frequency)
                writer.add_image('Train/Left Cap1_next', cap1_next[0]**(1/2.2), total_steps/training_frequency)
                ...
                log_multiple_feature_map(writer, fmap1, "Train/Unwarped_Fmap1_L", total_steps/training_frequency, num_channels=1)
                log_multiple_feature_map(writer, fmap1_next, "Train/Unwarped_Fmap2_L", total_steps/training_frequency, num_channels=1)
                log_multiple_feature_map(writer, warped_fmap_left, "Train/Warped_Fmap2", total_steps/training_frequency, num_channels=1)
                log_multiple_feature_map(writer, fused_fmap1, "Train/Fused_Fmap", total_steps/training_frequency, num_channels=1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='disp_recon_feature_fusion', help="name your experiment")

...
def train(args):
    ...
    while should_keep_training:
        ...
        for i_batch, (img_list, *data_blob) in enumerate(tqdm(train_loader)):
            optimizer.zero_grad()
            print(f"img_list : {img_list}, img_list_len : {len(img_list)}")

            img1_left, img1_right, img2_left, img2_right, lidar_gt = [x.cuda() for x in data_blob]
            print(f"img1_left shape : {img1_left.shape}")
            ...
            with torch.no_grad():
                disp_predictions, fmap1, fmap1_next, warped_fmap_left, flow_L, fused_fmap1, cap_img_list = model(img1_left, img1_right, img2_left, img2_right, exp1, exp2)
            ...
            print(disp_predictions[-1].shape, lidar_gt.shape)
            ...
            if total_steps % training_frequency == training_frequency - 1:
                ...
                writer.add_image('Train/Left_F1', img1_left[0]/255.0, total_steps/training_frequency)
                # writer.add_image('Train/Right_F1', img1_right[0]**(1/2.2), total_steps/training_frequency)
                writer.add_image('Train/Left_F2', img2_left[0]/255.0, total_steps/training_frequency)
                ...
                writer.add_image('Train/GT_disparity', visualize_lidar_with_colorbar(lidar_gt, vmin=0, vmax=lidar_gt.max()), total_steps/training_frequency)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', default='disp_fusion_mask_kitti', help="name your experiment")

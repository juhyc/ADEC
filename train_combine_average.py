...
# # * Model code : combine_model3.py (Average module + Disparity recon)
# # ! Finetuning with AverageAE module
# ###############################################

# Initialize writer for tensorboard logging
writer = SummaryWriter('runs/combine_pipeline_carla_average')
...
#* Training code
def train(args):
    model = torch.nn.DataParallel(CombineModel_w_averageAE(args), device_ids=[0,1])
    ...
    while should_keep_training:
        ...
        for i_batch, (_, *data_blob) in enumerate(tqdm(train_loader)):
            ...
            # Model forward pass
            fused_disparity, exp_average, left_hdr, captured_rand_img_list, captured_adj_img_list= model(
            ...
            if total_steps % training_frequency == training_frequency - 1:
                ...
                print(f"=========NAE_exposure_output : {exp_average}, training_frequency : {total_steps}//{args.num_steps}")
...
if __name__  == '__main__':
    ...
    parser.add_argument('--name', default='combine_model_average', help="name your experiment")

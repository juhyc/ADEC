...
###############################################
# * Build additional module on RAFT-stereo model
###############################################
...
class RAFTStereoFusion(nn.Module):
    def __init__(self, args):
        ...
        self.attention_fusion = AttentionFusion(in_planes=512)
    ...
    def forward(self, image1, image2, image1_warped, image2_warped, iters=32, flow_init=None, test_mode=False):
        """ Estimate optical flow between pair of frames """
        # Normalzie input image
        image1 = (2 * (image1 / 255.0) - 1.0).contiguous()
        image2 = (2 * (image2 / 255.0) - 1.0).contiguous()
        image1_warped = (2 * (image1_warped / 255.0) - 1.0).contiguous()
        image2_warped = (2 * (image2_warped / 255.0) - 1.0).contiguous()
        # image1 = (2 * image1 - 1.0).contiguous()
        # image2 = (2 * image2 - 1.0).contiguous()

        # Extract Feature
        fmap1, fmap2 = self.fnet([image1, image2])
        fmap1_warped, fmap2_warped = self.fnet([image1_warped, image2_warped])
        
        # Feautre Fusion
        fused_fmap1 = self.attention_fusion(fmap1, fmap1_warped)
        fused_fmap2 = self.attention_fusion(fmap2, fmap2_warped)

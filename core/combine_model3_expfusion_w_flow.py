...
################################
# * Exposure fusion (Pre-fusion) with motion compensation
################################
...
class CombineModel_expfusion_w_flow(nn.Module):
    ...
    def forward(self, left_hdr, right_hdr, left_next_hdr, right_next_hdr, initial_exp_high, initial_exp_low, iters=32):
        ...
        #* 2. Image-based warping
        warped_img_left = self.warp_image(ldr_left_expl_cap, flow_left)
        warped_img_right = self.warp_image(ldr_right_expl_cap, flow_right)
        
        # Mask 생성 및 hole 처리
        mask_left = (warped_img_left > 0).float()  # 유효한 픽셀만 1로 설정
        mask_right = (warped_img_right > 0).float()

        # Hole 처리: mask가 0인 경우 첫 번째 프레임의 값을 사용
        fused_left_image = mask_left * (ldr_left_exph_cap + warped_img_left) / 2.0 + (1 - mask_left) * ldr_left_exph_cap
        fused_right_image = mask_right * (ldr_right_exph_cap + warped_img_right) / 2.0 + (1 - mask_right) * ldr_right_exph_cap

        #* 3. Disparity reconstruction
        disp_predictions = self.disp_recon_net(fused_left_image, fused_right_image)

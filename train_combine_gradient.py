...
# # * Model code : combine_model3.py (Gradient module + Disparity recon)
# # ! Finetuning with GraidnetAEC module
# ###############################################

# Initialize writer for tensorboard logging
writer = SummaryWriter('runs/combine_pipeline_carla_gradient')
...
#* Training code
def train(args):
    model = torch.nn.DataParallel(CombineModel_w_gradientAE(args), device_ids=[0])
    ...
    while should_keep_training:
        ...
        for i_batch, (_, *data_blob) in enumerate(tqdm(train_loader)):
            ...
            # Model forward pass
            fused_disparity, exp_gradient, left_hdr, captured_rand_img_list, captured_adj_img_list= model(
            ...
            if total_steps % training_frequency == training_frequency - 1:
                ...
                print(f"=========graidnetAE_disparity_loss : {disparity_loss.item()}, training_frequency : {total_steps}//{args.num_steps}")
                print(f"=========Initial_exposure : {initial_random_exp}, training_frequency : {total_steps}//{args.num_steps}")
                print(f"=========graidnetAE_exposure_output : {exp_gradient}, training_frequency : {total_steps}//{args.num_steps}")
...
if __name__  == '__main__':
    ...
    parser.add_argument('--name', default='combine_model_gradient', help="name your experiment")
